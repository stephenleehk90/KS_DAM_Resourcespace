...
namespace DAM_Export
{
    public partial class frmMain : Form
    {
        ...
        {
            if (this.rd_SQL.Checked)
                return string.Format("Data Source={0};Initial Catalog={1};Persist Security Info=True;User ID={2};Password={3}", this.txt_Server.Text.Trim(), this.txt_DBName.Text.Trim(), this.txt_Login.Text.Trim(), this.txt_Pass.Text.Trim());
            else
                return string.Format("Integrated Security=SSPI;Persist Security Info=False;Data Source={0};Initial Catalog={1}", this.txt_Server.Text.Trim(), this.txt_DBName.Text.Trim());
        ...
        private bool ValidateData()
        {
            ...
            if (strApp.ToLower().EndsWith("dam_migration.exe"))
            {
                if (string.IsNullOrEmpty(this.txt_Server.Text.Trim()))
                {
                    MessageBox.Show("Database Server is empty!");
                    return false;
                }
                if (string.IsNullOrEmpty(this.txt_DBName.Text.Trim()))
                {
                ...
                if (this.rd_SQL.Checked)
                {
                    if (string.IsNullOrEmpty(this.txt_Login.Text.Trim()))
                    {
                        MessageBox.Show("Login Name is empty!");
                        return false;
                    }
                    if (string.IsNullOrEmpty(this.txt_Pass.Text.Trim()))
                    {
            ...
            {
                MessageBox.Show("Linux Server IP is empty!");
                return false;
            }
            if (string.IsNullOrEmpty(this.txt_LinuxPort.Text.Trim()))
            {
                MessageBox.Show("Linux Server Port is empty!");
                return false;
            }
            if (string.IsNullOrEmpty(this.txt_LinuxLogin.Text.Trim()))
            {
                MessageBox.Show("Linux Login Name is empty!");
                return false;
            }
            if (string.IsNullOrEmpty(this.txt_LinuxPass.Text.Trim()))
            {
                MessageBox.Show("Linux Login Password is empty!");
                return false;
            }
        ...
        private bool Test_Conn(bool bMsg)
        {
            //  OleDbConnection conn_ole = new OleDbConnection(connection);
            try
            {
                m_conn = new SqlConnection(this.GetConnectionString());
                m_conn.Open();
                //
                m_conn.Close();
                if (bMsg)
        ...
        {
          txt_Login.Enabled = rd_SQL.Checked;
      
            txt_Pass.Enabled = rd_SQL.Checked;
        }
...
//            MessageBox.Show();
            ...
            if (strApp.ToLower().EndsWith("dam_migration.exe"))
            {
                ...
                this.Text = "DAM Migration Tool";
                cbDate.SelectedIndex = 0;
                txt_DBName.Text = setting.strDatabase;
                txt_Server.Text = setting.strDBServer;
                txt_Pass.Text = setting.strDBPassword;
                txt_Login.Text = setting.strDBUserName;
                if (setting.strType == "0" || setting.strType == "")
                {
                    rd_SQL.Checked = true;
                    rd_Windows.Checked = false;
                }
                else
                {
                    rd_SQL.Checked = false;
                    rd_Windows.Checked = true;
                }

              //  dtpStart.Value = setting.dtStart;
              //  dtpEnd.Value = setting.dtEnd;
                txt_OutputDir.Text = setting.strOutputDir;
                txt_SourceDir.Text = setting.strSourceDir;
            ...
            else
            {
                ...
                this.Text = "DAM Data Upload Tool";
        ...
        private bool Apply_DB_Setting(bool bMsg, bool bValidate)
        {
            try
            {
                ...
                if (strApp.ToLower().EndsWith("dam_migration.exe"))
                {
                    setting.strDatabase = txt_DBName.Text.Trim();
                    setting.strDBServer = txt_Server.Text.Trim();
                    setting.strDBPassword = txt_Pass.Text.Trim();
                    setting.strDBUserName = txt_Login.Text.Trim();
                    if (rd_SQL.Checked)
                        setting.strType = "0";
                    else
                        setting.strType = "1";
                ...
                if (strApp.ToLower().EndsWith("dam_migration.exe")==false)
                {
                    LoadUserCollectionResourceTypes(false, false);
                }
                if (bMsg) 
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Test_Conn(false)==false) return;
                if (Test_Linux(false)==false) return;

                if (string.IsNullOrEmpty(this.txt_OutputDir.Text.Trim()))
                {
                    MessageBox.Show("CSV Output Directory is empty!");
                    return;
                }

                if (Directory.Exists(txt_OutputDir.Text.Trim())==false)
                {
                    MessageBox.Show("CSV Output Directory not exists!");
                    return;
                }

                if (string.IsNullOrEmpty(this.txt_SourceDir.Text.Trim()))
                {
                    MessageBox.Show("Source Directory is empty!");
                    return;
                }

                if (Directory.Exists(txt_SourceDir.Text.Trim()) == false)
                {
                    MessageBox.Show("Source Directory is not existed!");
                    return;
                }



                if (string.IsNullOrEmpty(this.txt_Root.Text.Trim()))
                {
                    MessageBox.Show("Linux Server Files Copy Directory is empty!");
                    return;
                }


                string strPath = txt_OutputDir.Text.Trim() + "\\output_" + dtpStart.Value.ToString("ddMMyyyy") + "_" +
                    dtpEnd.Value.ToString("ddMMyyyy") + ".csv";

                FileStream fs = new FileStream(strPath, FileMode.Create, FileAccess.Write, FileShare.Write);
                fs.Close();
                fs = null;
                Encoding utf8WithoutBom = new UTF8Encoding(encoderShouldEmitUTF8Identifier: false);
                //StreamWriter sw = new StreamWriter(strPath, true, utf8WithoutBom);
                StreamWriter sw = new StreamWriter(strPath, true, Encoding.Default);
                //                string NextLine = "Original filename,Title,Creator,Pub Date,Date,Category,Format,Recorder,HaveAssociate,Resource type,status,access,Location,Related Asset";
                string NextLine = "Filename,Original filename,Title,Caption,Creator,Pub Date,Date,Collection,Recorder,Resource Type,Department,Page,File Path,Related Resource";
                sw.WriteLine(NextLine);


                using (m_conn = new SqlConnection(this.GetConnectionString()))
                {
                    m_conn.Open();

                    SqlCommand arithabortCommand = new SqlCommand("SET ARITHABORT ON", m_conn);
                    arithabortCommand.ExecuteNonQuery();
/*
                    string strSQL = "Select af.FileName, a.Title, a.Creator, Convert(varchar, a.Date,120) as Date, Convert(varchar, a.RecordCreationDate,120) as RecordCreationDate, sa.Name as Category, a.Format, a.Recorder, a.HaveAssociate, 0 as status, 0 as access, ";
                    strSQL = strSQL + " fl.path, ssa.related_asset from app_AssetBasicInfo a LEFT OUTER JOIN sys_AssetCategory sa on sa.Id = a.CategoryID ";
                    strSQL = strSQL + " LEFT outer JOIN ";
                    strSQL = strSQL + " ( ";
                    strSQL = strSQL + " SELECT DISTINCT aa2.AssetID, ";
                    strSQL = strSQL + " ( "; 
                    strSQL = strSQL + " SELECT CONVERT(varchar, af.FileName) + ',' AS [text()] ";
	                strSQL = strSQL + " FROM app_AssetAssociate aa1, app_files af, app_AssetFiles asf ";
	                strSQL = strSQL + " WHERE aa1.AssetID = aa2.AssetID ";
	                strSQL = strSQL + " AND af.ID = asf.FileID ";
                    strSQL = strSQL + " AND aa1.AssociateAssetID = asf.AssetID ";
	                strSQL = strSQL + " ORDER BY aa1.AssetID, aa1.AssociateAssetID ";
                    strSQL = strSQL + " FOR XML PATH (''), TYPE ).value('text()[1]','nvarchar(max)') [related_asset] ";
                    strSQL = strSQL + " FROM app_AssetAssociate aa2 ";
                    strSQL = strSQL + " ) ssa on ssa.AssetID = a.Id, ";


                    strSQL = strSQL + " app_files af, app_FileLocation fl, app_AssetFiles aaf ";
                    strSQL = strSQL + " where aaf.AssetID = a.Id and aaf.FileID = fl.FileID and aaf.FileID = af.Id and"; 
                    strSQL = strSQL + " CONVERT(DATETIME, a.Date) between '" 
                        + dtpStart.Value.ToString("yyyy-MM-dd") + "' and '" + dtpEnd.Value.ToString("yyyy-MM-dd") + "'";
                    strSQL = strSQL + " union Select af.FileName, a.DisplayName, a.recorder, Convert(varchar, a.recorddate,120), Convert(varchar, a.recorddate,120), '' as Category, '' as Format, a.recorder, 0 as HaveAssociate, 0 as status, 0 as access, ";
                    strSQL = strSQL + " fl.path, '' as related_asset from app_AssetAccessory a, ";
                    strSQL = strSQL + " app_files af, app_FileLocation fl, app_AccessoryFiles aaf ";
                    strSQL = strSQL + " where aaf.AccessoryID = a.Id and aaf.FileID = fl.FileID and aaf.FileID = af.Id and";
                    strSQL = strSQL + " DATEADD(dd, DATEDIFF(dd, 0, recorddate), 0)  between '"
                        + dtpStart.Value.ToString("yyyy-MM-dd") + "' and '" + dtpEnd.Value.ToString("yyyy-MM-dd") + "' and lower(FileName) not like '%_web.jpg'";
*/     

                    string strSQL = "select w.*, c3.ks_dept as txt_dept, colname as txt_col, c3.editionnum as txt_num, ";
                    strSQL = strSQL + " c4.ks_dept as pic_dept, c4.ks_page as pic_col, c4.editionnum as pic_num, ";
                    strSQL = strSQL + " c5.ks_dept as pic_dept1, c5.ks_page as pic_col1, pc.photo_cap ";
                    strSQL = strSQL + " from ";
                    strSQL = strSQL + " (Select a.Id, af.FileName, REPLACE(REPLACE(a.Title, CHAR(13), ' '), CHAR(10), ' ') as Title, a.Creator, Convert(varchar, a.Date,120) as Date, "; 
                    strSQL = strSQL + " Convert(varchar, a.RecordCreationDate,120) as RecordCreationDate, sa.Name as Category, ";
                    strSQL = strSQL + " a.Format, a.Recorder, a.HaveAssociate, 0 as status, 0 as access,  fl.path, ssa.related_asset, af.OriginalName ";
                    strSQL = strSQL + "  from app_AssetBasicInfo a ";
                    strSQL = strSQL + " LEFT OUTER JOIN sys_AssetCategory sa on sa.Id = a.CategoryID "; 
                    strSQL = strSQL + " LEFT outer JOIN  (  SELECT DISTINCT aa2.AssetID, "; 
                    strSQL = strSQL + " (  SELECT CONVERT(varchar, af.FileName) + ',' AS [text()]  FROM app_AssetAssociate aa1, ";
                    strSQL = strSQL + " app_files af, app_AssetFiles asf  WHERE aa1.AssetID = aa2.AssetID  AND af.ID = asf.FileID ";  
                    strSQL = strSQL + " AND aa1.AssociateAssetID = asf.AssetID  ORDER BY aa1.AssetID, aa1.AssociateAssetID";  
                    strSQL = strSQL + " FOR XML PATH (''), TYPE ).value('text()[1]','nvarchar(max)') [related_asset] ";  
                    strSQL = strSQL + " FROM app_AssetAssociate aa2  ) ssa on ssa.AssetID = a.Id, "; 
                    strSQL = strSQL + " app_files af, app_FileLocation fl, app_AssetFiles aaf ";
                    strSQL = strSQL + "  where aaf.AssetID = a.Id and aaf.FileID = fl.FileID and aaf.FileID = af.Id ";
                    strSQL = strSQL + " and CONVERT(DATETIME, a.Date) ";
                    strSQL = strSQL + " between '" + dtpStart.Value.ToString("yyyy-MM-dd") + " 00:00:00.000' and '" + dtpEnd.Value.ToString("yyyy-MM-dd") + " 23:59:59.998'";
                    strSQL = strSQL + " and lower(fl.path) not like '%asset%' ";
                    strSQL = strSQL + " union "; 
                    strSQL = strSQL + " Select a.Id, af.FileName, a.DisplayName, a.recorder, Convert(varchar, a.recorddate,120) as Date, "; 
                    strSQL = strSQL + " Convert(varchar, a.recorddate,120), '' as Category, '' as Format, a.recorder, 0 as HaveAssociate, ";
                    strSQL = strSQL + " 0 as status, 0 as access,  fl.path, '' as related_asset, af.OriginalName from app_AssetAccessory a, ";  
                    strSQL = strSQL + " app_files af, app_FileLocation fl, app_AccessoryFiles aaf  where aaf.AccessoryID = a.Id "; 
                    strSQL = strSQL + " and aaf.FileID = fl.FileID and aaf.FileID = af.Id and "; 
                    strSQL = strSQL + " CONVERT(DATETIME, a.recorddate) ";  
                    strSQL = strSQL + " between '" + dtpStart.Value.ToString("yyyy-MM-dd") + " 00:00:00.000' and '" + dtpEnd.Value.ToString("yyyy-MM-dd") + " 23:59:59.998'";
                    strSQL = strSQL + " and lower(FileName) not like '%_web.jpg' ";
                    strSQL = strSQL + " and lower(fl.path) not like '%asset%' ";
                    strSQL = strSQL + " ) w ";
                    strSQL = strSQL + "  left outer join app_AssetCustomInfo2 c3 on w.Id = c3.Id ";
                    strSQL = strSQL + "  left outer join (select AssociateAssetID, ai2.ks_dept, ai2.ks_page, ai2.editionnum ";
                    strSQL = strSQL + " from app_AssetAssociate asco, "; 
                    strSQL = strSQL + " app_AssetCustomInfo2 ai2, app_AssetCustomInfo3 ai3 ";
                    strSQL = strSQL + " where ai2.Id = asco.AssetID and ai3.Id = asco.AssociateAssetID) c4 on w.Id = c4.AssociateAssetID ";
                    strSQL = strSQL + "  left outer join app_AssetCustomInfo3 c5 on w.Id = c5.Id ";
                    strSQL = strSQL + "  left outer join (select id, Description as photo_cap from app_AssetBasicInfoExt where CategoryId = 3 ) pc on w.Id = pc.id ";
                    strSQL = strSQL + " order by Date, FileName ";
                    ...
                    string strFTPPath = txt_Root.Text;
                    if (!strFTPPath.EndsWith("/"))
                        strFTPPath = strFTPPath + "/";
                    bool fileExist = sftpclient.Exists(strFTPPath);
                    if (!fileExist)
                    {
                        sftpclient.CreateDirectory(strFTPPath);
                        SftpFileAttributes attr1 = sftpclient.GetAttributes(strFTPPath);
                        attr1.SetPermissions(777);
                        sftpclient.SetAttributes(strFTPPath, attr1);
                    }
...
//                                string strResType = GetResourceTypeID(dr["Format"].ToString());
                                
                                if (dr["Format"].ToString() == "jpg" || dr["Format"].ToString() == "gif" || dr["Format"].ToString() == "eps" || dr["Format"].ToString() == "tif")
                                    strResType = "1";
                                if (dr["Format"].ToString() == "txt")
                                    strResType = "2";
                                else if (dr["Format"].ToString() == "pdf" || dr["Format"].ToString() == "xml")
                                    strResType = "2";
                                else if (dr["Format"].ToString() == "mov" || dr["Format"].ToString() == "mpeg" || dr["Format"].ToString() == "mp4" || dr["Format"].ToString() == "mpg")
                                    strResType = "2";
                                else if (dr["Format"].ToString() == "mp3" || dr["Format"].ToString() == "wav")
                                        strResType = "2";
...
//                                string strStatus = "Active";
                                ...
                                string strFilePath = dr["path"].ToString().Trim().Replace("/", "\\");
                                if (strFilePath.EndsWith("\\")) 
                                    strFilePath = strFilePath + dr["FileName"].ToString().Trim();
                                else
                                    strFilePath = strFilePath + "\\" + dr["FileName"].ToString().Trim();

                                string strRoot = txt_Root.Text.Trim();
                                if (!strRoot.EndsWith("/"))
                                    strRoot = strRoot + "/";

                                string strResouceSpaceUploadPath = strRoot + strFilePath.Replace("\\","/");
                                string strResouceSpacePath = strFilePath.Replace("\\", "/");

                                string strDept = dr["txt_dept"].ToString();
                                string strCol = dr["txt_col"].ToString();
                                string strNum = dr["txt_num"].ToString();

                                if (strDept.Trim()=="")
                                    strDept = dr["pic_dept"].ToString();
                                if (strCol.Trim() == "")
                                    strCol = dr["pic_col"].ToString();
                                if (strNum.Trim() == "")
                                    strNum = dr["pic_num"].ToString();

                                if (strDept.Trim() == "")
                                    strDept = dr["pic_dept1"].ToString();
                                if (strCol.Trim() == "")
                                    strCol = dr["pic_col1"].ToString();
                                if (strNum.Trim() == "")
                                {
                                    try
                                    {
                                        if (dr["pic_num1"] != null )
                                            strNum = dr["pic_num1"].ToString();
                                    }
                                    catch (Exception ee)
                                    {
                                    }
                                ...
                                string str_Dept = strDept.Trim() + "/" + ReplaceNewDept(strCol.Trim());
                                if (str_Dept.Trim() == "/" || str_Dept.Trim().StartsWith("/") || str_Dept.Trim().EndsWith("/"))
                                    str_Dept = str_Dept.Replace("/", "");

                                NextLine = dr["FileName"].ToString()
                                    + "," + dr["OriginalName"].ToString()
                                    + "," + dr["Title"].ToString().Replace(",", " ")
                                    + "," + dr["photo_cap"].ToString().Replace(",", " ").Replace("／", "$^^-$^^-$^^-")
                                    + "," + dr["Creator"].ToString().Replace(",", " ")
                                    + "," + dr["Date"].ToString()
                                    + "," + dr["RecordCreationDate"].ToString()
                                    + "," + dr["Category"].ToString()
//                                    + "," + dr["Format"].ToString()
                                    + "," + dr["Recorder"].ToString().Replace(",", " ")
//                                    + "," + dr["HaveAssociate"].ToString()
                                    + "," + strResType
                                    //+ "," + strDept.Trim() + "/" + ReplaceNewDept(strCol.Trim())
                                    + "," + str_Dept.Trim()
                                    //                                    + "," + strDept.Trim() + "/" + strCol.Trim()
                                    + "," + strNum.Trim()
                                    //                                  + "," + strStatus
  //                                  + "," + dr["access"].ToString()
                                    + "," + strResouceSpacePath
                                    + "," + dr["related_asset"].ToString().Replace(",", "|");
                                sw.WriteLine(NextLine);
                                ...
                                try
                                {
                                    ...
                                    if (txt_SourceDir.Text.Trim().EndsWith("\\"))
                                        localFilename = txt_SourceDir.Text + strFilePath;
                                    else
                                       localFilename = txt_SourceDir.Text + "\\" + strFilePath;

                                    string strTempPath = dr["path"].ToString().Trim().Replace("\\", "/");
                                    string[] subpaths = strTempPath.Split('/');
                                    string strTmpPath = strRoot;

                                    foreach (string subpath in subpaths)
                                    {
                                        strTmpPath = strTmpPath + subpath + "/";

                                        fileExist = sftpclient.Exists(strTmpPath);
                                        if (!fileExist)
                                        {
                                            sftpclient.CreateDirectory(strTmpPath);
                                            SftpFileAttributes attr1 = sftpclient.GetAttributes(strTmpPath);
                                            attr1.SetPermissions(777);
                                            sftpclient.SetAttributes(strTmpPath, attr1);
...
//                                    using (var fileStream = File.OpenRead(localFilename))
  //                                  {
                                    if (localFilename.ToLower().EndsWith(".txt"))
                                    {
                                        Encoding enc = GetEncoding(localFilename);

                                        string fileContent = File.ReadAllText(localFilename, Encoding.Default);
...
//                                        var writer = new StreamWriter(stream, Encoding.UTF8);
  //                                       var writer = new StreamWriter(stream, utf8WithoutBom);
                                        ...
                                        sftpclient.UploadFile(stream, strResouceSpaceUploadPath);

                                        SftpFileAttributes attr1 = sftpclient.GetAttributes(strResouceSpaceUploadPath);
                                        attr1.SetPermissions(777);
                                        sftpclient.SetAttributes(strResouceSpaceUploadPath, attr1);
                                    ...
                                    else
                                    {
                                        using (var fileStream = File.OpenRead(localFilename))
                                       {
                                           ...
                                           sftpclient.UploadFile(fileStream, strResouceSpaceUploadPath);
                                           SftpFileAttributes attr1 = sftpclient.GetAttributes(strResouceSpaceUploadPath);
                                           attr1.SetPermissions(777);
                                           sftpclient.SetAttributes(strResouceSpaceUploadPath, attr1);
                                ...
                                {
                                    Global.WriteToFile("Copy File to Server Error: " +  ex.Message, false);
                                }
                            }
                        }
                    }
...
*/                
        ...
        private void frmMain_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (strApp.ToLower().EndsWith("dam_migration.exe"))
            {
                setting.strOutputDir = txt_OutputDir.Text.Trim();
                setting.strSourceDir = txt_SourceDir.Text.Trim();
                setting.dtStart = dtpStart.Value.Date;
                setting.dtEnd = dtpEnd.Value.Date;
            }
            settingClass.SaveSetting(setting);
        }
...
//            if (LoadUserCollectionResourceTypes(false, true) == false) return; 
            ...
            {
                MessageBox.Show("Resourcespace URL is empty!");
                return;
            }

            if (txt_RS_Login.Text.Trim() == "")
            {
                MessageBox.Show("Resourcespace Login is empty!");
                return;
            }
            ...
            {
                MessageBox.Show("Source Files Directory is empty!");
                return;
            }

            if (!Directory.Exists(txt_RS_SourceDir.Text))
            {
                MessageBox.Show("Source Files Directory is not existed!");
                return;
            }
...
//                Application.Exit();
        ...
        private void btn_Dir_Src_Click(object sender, EventArgs e)
        {
            FolderBrowserDialog dlg = new FolderBrowserDialog();
...
//                                sadsaddsd = sadsaddsd + "t/b";
  //                              sadsaddsd = sadsaddsd + "it";
    //                            sadsaddsd = sadsaddsd + "na";
      //                          sadsaddsd = sadsaddsd + "mi/php/etc";

                                //sadsaddsd = sadsaddsd + "ce/plugins/csv_upload/pages";
                                //                                sadsaddsd = sadsaddsd + "ce/api";
//                                sadsaddsd = sadsaddsd + "ce/pages/team";
                
                
                if (!sadsaddsd.EndsWith("/"))
                    sadsaddsd = sadsaddsd + "/";

                DownloadDirectory(jfvdfvytrtwer, sadsaddsd, @"D:\\Work\\DAM_Export\\Download\\");
            }
            catch (Exception ex)
            {
...
//            DownloadDirectory(jfvdfvytrtwer, sadsaddsd, @"D:\\Work\\DAM_Export\\DAM_Export\\DAM_Export\\bin\\Debug\\dl\\");
            using (var fileStream = File.OpenRead(@"D:\\Work\\DAM_Export\\Modified\\csv_functions.php"))
            {
                jfvdfvytrtwer.UploadFile(fileStream, sadsaddsd + "csv_functions.php");
            }

            using (var fileStream = File.OpenRead(@"D:\\Work\\DAM_Export\\Modified\\del_res.php"))
            {
                jfvdfvytrtwer.UploadFile(fileStream, sadsaddsd + "del_res.php");
            }
...
/*
            ...
            //            DownloadDirectory(jfvdfvytrtwer, sadsaddsd, @"D:\\Work\\DAM_Export\\DAM_Export\\DAM_Export\\bin\\Debug\\dl\\");
            using (var fileStream = File.OpenRead(@"D:\\Work\\DAM_Export\\Modified\\team_home.php"))
            {
                jfvdfvytrtwer.UploadFile(fileStream, sadsaddsd + "team_home.php");
            }
      ...
      //      }
          ...
          //  using (var fileStream = File.OpenRead(@"D:\\Work\\DAM_Export\\DAM_Export\\DAM_Export\\bin\\Debug\\dl\\api_bindings.php"))
           ...
           // }
            ...
            using (var fileStream = File.OpenRead(@"D:\\Work\\DAM_Export\\Modified\\search.php"))
            {
                jfvdfvytrtwer.UploadFile(fileStream, sadsaddsd + "search.php");
            }
