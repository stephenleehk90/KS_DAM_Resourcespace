...
namespace DAM_Export
{
    class SettingClass
    {
        ...
        public void SaveSetting(Setting setting)
        {
            ...
            {
                strValue = setting.strDBServer;
                config.UpdateKeyValue(strValue, "DBServer");
                strValue = "";
                strValue = setting.strDBUserName;
                config.UpdateKeyValue(strValue, "DBUsername");
                strValue = "";
                strValue = PasswordEncryption(setting.strDBPassword);
                config.UpdateKeyValue(strValue, "DBPassword");
                strValue = "";
                strValue = setting.strDatabase;
                config.UpdateKeyValue(strValue, "DatabaseName");
                strValue = "";
                strValue = setting.strType;
                config.UpdateKeyValue(strValue, "ConnectType");

                strValue = "";
                strValue = setting.strOutputDir;
                config.UpdateKeyValue(strValue, "OutputDir");

                strValue = "";
                strValue = setting.strSourceDir;
                config.UpdateKeyValue(strValue, "SourceDir");

                strValue = "";
                strValue = setting.dtStart.ToString("dd/MM/yyyy");
                config.UpdateKeyValue(strValue, "StartDate");

                strValue = "";
                strValue = setting.dtEnd.ToString("dd/MM/yyyy");
                config.UpdateKeyValue(strValue, "EndDate");
            }
            else
            {
                strValue = "";
...
/*
  ...
  */      
        ...
        private void LoadSetting(String strViewName)
        {
            ...
            if (strApp.ToLower().EndsWith("dam_migration.exe"))
            {
                strTemp = config.GetKeyValue("DBServer");
                setting.strDBServer = strTemp;
                strTemp = config.GetKeyValue("DBUsername");
                setting.strDBUserName = strTemp;
                strTemp = config.GetKeyValue("DBPassword");
                setting.strDBPassword = PasswordDecryption(strTemp);
                strTemp = config.GetKeyValue("DatabaseName");
                setting.strDatabase = strTemp;
                strTemp = config.GetKeyValue("ConnectType");
                setting.strType = strTemp;

                strTemp = config.GetKeyValue("StartDate");
                DateTime dateStart;
                if (DateTime.TryParseExact(strTemp, "dd/MM/yyyy",
                                           CultureInfo.InvariantCulture,
                                           DateTimeStyles.None,
                                           out dateStart))
                    setting.dtStart = dateStart;
                else
                    setting.dtStart = DateTime.Now.Date;

                strTemp = config.GetKeyValue("EndDate");
                DateTime dateEnd;
                if (DateTime.TryParseExact(strTemp, "dd/MM/yyyy",
                                           CultureInfo.InvariantCulture,
                                           DateTimeStyles.None,
                                           out dateEnd))
                    setting.dtEnd = dateEnd;
                else
                    setting.dtEnd = DateTime.Now.Date;
            
                strTemp = config.GetKeyValue("OutputDir");
                setting.strOutputDir = strTemp;
                strTemp = config.GetKeyValue("SourceDir");
                setting.strSourceDir = strTemp;
            }
            else
            {
